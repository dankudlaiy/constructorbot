...
curl -s "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN_COLDBET}/setWebhook?url=${APP_URL}/miniapp/webhook.php?brand=coldbet" | python3 -m json.tool
echo ""

echo "SpinBetter webhook:"
curl -s "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN_SPINBETTER}/setWebhook?url=${APP_URL}/miniapp/webhook.php?brand=spinbetter" | python3 -m json.tool
echo ""
